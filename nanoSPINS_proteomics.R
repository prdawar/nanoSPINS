...
protein_input %>%
  ...
  ggplot() +
  aes(x = fct_reorder(Group, n), y = n) +
  geom_boxplot(aes(colour = Group), outlier.size = -1, 
...
protein_input_750wide <- protein_input %>%
  filter(!is.na(Intensity)) %>%
  mutate(Channel = paste(Group, 
                         str_sub(SampleID,-1,-1), 
                         sep = "_")) %>%
  group_by(SampleID) %>%
  add_count(name = "n") %>%
...
protein_input_750wide %>%
  ...
  mutate(Group = case_when(grepl("C10", SampleID) ~ 
                           ...
                           grepl("SVEC", SampleID) ~ 
                             "SVEC")) %>%
  mutate(Batch = str_match(SampleID, 
                           "_\\s*(.*?)\\s*_")) %>%
  mutate(Batch = Batch[,2]) %>%
  filter(!is.na(Intensity)) %>%
...
median_CVs <- protein_input_750wide %>% 
  ...
  pivot_longer(!Gene, names_to = "SampleID", 
               values_to = "Intensity") %>% 
  mutate(Group = case_when(grepl("C10", 
                                 SampleID) ~  "C10", 
                           grepl("SVEC", 
                                 SampleID) ~ "SVEC")) %>% 
  mutate(Batch = str_match(SampleID, 
...
protein_input_750long <- protein_input_750wide %>% 
  rownames_to_column(var = "Gene") %>% 
  pivot_longer(!Gene, names_to = "SampleID", 
               values_to = "Intensity") %>% 
  mutate(Group = case_when(grepl("C10", 
...
length(unique(protein_input_750long$SampleID)[grepl("C10", unique(protein_input_750long$SampleID))])
length(unique(protein_input_750long$SampleID)[grepl("SVEC", unique(protein_input_750long$SampleID))])
...
thirty_percent_cutoff <- protein_input_batch_corrected %>% 
  rownames_to_column(var = "Gene") %>%
  pivot_longer(!Gene, names_to = "SampleID", values_to = "Intensity") %>%
  mutate(Group = case_when(grepl("C10", SampleID) ~ "C10",
                           grepl("SVEC", SampleID) ~ "SVEC")) %>%
...
fifty_percent_cutoff <- protein_input_batch_corrected %>% 
  rownames_to_column(var = "Gene") %>%
  pivot_longer(!Gene, names_to = "SampleID", values_to = "Intensity") %>%
  mutate(Group = case_when(grepl("C10", SampleID) ~ "C10",
                           grepl("SVEC", SampleID) ~ "SVEC")) %>%
...
seventy_percent_cutoff <- protein_input_batch_corrected %>% 
  rownames_to_column(var = "Gene") %>%
  pivot_longer(!Gene, names_to = "SampleID", values_to = "Intensity") %>%
  mutate(Group = case_when(grepl("C10", SampleID) ~ "C10",
                           grepl("SVEC", SampleID) ~ "SVEC")) %>%
...
pca_protein_input_impute$metadata <- meta
