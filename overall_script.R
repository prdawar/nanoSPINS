...
TMT_impute %>%
  ...
  xlab("")+  scale_y_continuous(limits = c(0,2)) +
  geom_violin() + 
  #geom_jitter() +
  ...
  #facet_wrap(~Batch) + 
  theme_minimal(base_size = 24) + ggtitle("coefficient of variation")
...
test5 %>%
  ...
  #filter(!is.na(CV)) %>%
  ggplot()+
  aes(x = celltype, y = CV, fill = celltype)+
  ylab("Coefficient of Variation")+
  #xlab("")+  
  ggtitle("") +
...
DimPlot(C10SVEC_rawdata_new, label = TRUE, reduction = "umap",
        group.by = "celltype")
...
C10SVEC_rawdata_new <- FindMultiModalNeighbors(
  C10SVEC_rawdata_new, reduction.list = list("pca", "apca"), 
  dims.list = list(1:4, 1:10),
  knn.range = 50, smooth = FALSE,
...
p1 <- DimPlot(C10SVEC_rawdata_new, group.by = "celltype",
              reduction = 'wnn.umap',
              label = TRUE, 
              repel = TRUE,
...
p3 <- DimPlot(C10SVEC_rawdata_new, reduction = 'rna.umap', 
              group.by = "celltype",
              label = TRUE, 
              repel = TRUE, label.size = 5) + NoLegend()
p4 <- DimPlot(C10SVEC_rawdata_new, group.by = "celltype",
...
C10SVEC_rawdata_new.markers_RNA <- FindAllMarkers(C10SVEC_rawdata_new, 
                                                  assay = "RNA",
                                                  only.pos = TRUE,
...
DoHeatmap(C10SVEC_rawdata_new, 
          group.by = "celltype", 
          assay  ="RNA", 
          features = as.character(top10_RNA_normalprocessing$gene), 
...
C10SVEC_rawdata_new.markers_prot <- FindAllMarkers(C10SVEC_rawdata_new, 
                                                   assay = "Prot",
                                                   only.pos = TRUE,
                                                   test.use = "wilcox_limma" ,
                                                   # min.diff.pct = 0.2, 
...
DoHeatmap(C10SVEC_rawdata_new, group.by = "celltype",
          assay  ="Prot", features = top10_Prot_normalprocessing_padj$gene, group.colors = c("gold", "seagreen"), label = TRUE, draw.lines = FALSE) + scale_fill_gradientn(colours = rev(colorpalette)) + guides(color="none")
...
normalized_scRNAseq_scT_RNA <- normalized_scRNAseq_scT %>% 
  setNames(paste0(names(.), "_RNA")) %>% rownames_to_column(var = "gene") %>%  mutate_all(~replace(., . == 0, NA))
...
normalized_scRNAseq_SCT_long_new <- normalized_scRNAseq_scT %>% rownames_to_column(var = "gene") %>%  pivot_longer(!gene, names_to = "SampleID", values_to = "TPM") 
#%>%  mutate(TPM = ifelse(TPM == 0, NA, TPM)) %>% filter(!is.na(TPM))

combined_long_new <- full_join(TMT_norm_long, normalized_scRNAseq_SCT_long_new) %>% filter(!is.na(SampleID)) %>% filter(!is.na(TPM)) %>% filter(!is.na(Intensity)) %>% mutate(Group = case_when(grepl("C10", SampleID) ~ "C10", grepl("SVEC", SampleID) ~ "SVEC")) 
...
test10 %>%
  mutate(padj = p.adjust(pvalue, n = nrow(test10), method = "BH")) %>%
  mutate(group = ifelse(padj < 0.05, "significant", "non-significant")) %>%
  ggplot(aes(x = correlation, 
             y = -log10(pvalue), 
             color = factor(group))) + 
  geom_point(na.rm = TRUE) + 
  geom_text_repel(data = . %>% filter(padj < 0.05), 
                  aes(label = gene), vjust = -5, 
                  direction = "x", size = 4, ) +
...
#ggsave("correlation_RNA_protein_non-imputed_C10_padj.png", width = 8, height = 6, bg = "white")
...
test12 %>%
  mutate(padj = p.adjust(pvalue, n = nrow(test12), method = "BH")) %>% 
  mutate(group = ifelse(padj < 0.05, "significant", "non-significant")) %>%
...
#ggsave("correlation_RNA_protein_non-imputed_padj_SVEC.png", width = 8, height = 6, bg = "white")
